...
<figure class="frame">
  <video controls loop autoplay="true" muted>
    <source src="/docs/media/v1/ui/clusters/connect-cluster-loft.mp4" type="video/mp4" />
  </video>
...
Since Loft already has access to the cluster it is running inside, you will see a special button <Highlight>Connect loft cluster</Highlight> which will let you connect the management cluster with a single click.
