...
to create a unique name. Optionally, the `--use` flag can be used to reuse an existing virtual cluster instead of creating a new one.
...
is deleted even if the tests fail. You may wish to skip this step if reusing the virtual cluster with the `--use` flag when creating the virtual cluster.