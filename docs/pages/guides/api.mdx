...
The Loft API Server has several endpoints that serve different purposes:
- **/kubernetes/management/**: Kubernetes conform api that targets the internal Loft management API.
- **/kubernetes/cluster/$CLUSTER/**: Endpoint that proxies incoming Kubernetes requests to the connected cluster $CLUSTER.
- **/kubernetes/virtualcluster/$CLUSTER/$NAMESPACE/$VCLUSTER/**: Endpoint that proxies incoming Kubernetes requests to the virtual cluster $VCLUSTER in namespace $NAMESPACE in the connected cluster $CLUSTER.
...
- **/grafana/$CLUSTER/** (Authentication Required): All requests to this endpoint will be proxied to the cluster's grafana instance (if there is any). Used by the Loft UI to show [grafana dashboards](./monitoring.mdx) in the monitoring tab.

### Access the Loft management API directly 

The Loft management API are virtual resources (not persisted in etcd) that facilitate interaction with Loft. These resources are primarily used by the Loft UI and CLI to change state in the cluster where Loft is installed in, without the need to have cluster wide access.
...
### Access a connected cluster through Loft directly
...
### Access a virtual cluster through Loft directly
...
In order to access a virtual cluster through Loft directly, you need access to the virtual cluster credentials. The easiest way to get these credentials is through the loft CLI:

```bash
$ loft use vcluster $VCLUSTER --cluster $CLUSTER --space $NAMESPACE --print-token
LS0tLS1CRUdJTiBDRVJUSUZJQ0FURS0tLS0tCk1JSUJjRENDQVJhZ0F3SUJBZ0lJW...
```

:::info Directly usable kube config
If you need a directly usable kubeconfig, you can also use:
```bash
$ loft use vcluster $VCLUSTER --cluster $CLUSTER --space $NAMESPACE --print
apiVersion: v1                                                    
clusters:
- cluster:
...
...
With the virtual cluster credentials `$CREDENTIALS` you can query a virtual cluster `$VCLUSTER` in the connected cluster `$CLUSTER` in the namespace / space `$NAMESPACE` by prefixing `/kubernetes/virtualcluster/$CLUSTER/$NAMESPACE/$VCLUSTER/`:
```bash
