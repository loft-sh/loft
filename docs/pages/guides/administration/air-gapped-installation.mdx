...
:::warning Open Ports
Since loft and kiosk install [webhooks](https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/) and [api server extensions](https://kubernetes.io/docs/tasks/extend-kubernetes/setup-extension-api-server/) into the cluster, the kubernetes master needs to be able to communicate with loft and kiosk.
In private GKE clusters for example this is not the default.
You need to ensure that incoming traffic from the kubernetes master to the following tcp ports is allowed:
