...
Enforced templates are part of the "Space Creation Settings" which means that they are only looked at during the creation of new spaces. Changing the enforced templates will **not** be reflected in existing space **nor** will template instances removed when a template is removed from the list of enforce templates.
:::
...
<figure class="frame">
  <video controls loop autoplay="true" muted>
    <source src="/img/ui/clusters/delete-template.mp4" type="video/mp4" />
  </video>
