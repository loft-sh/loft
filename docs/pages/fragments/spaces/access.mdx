...
Accessing an already existing space can be either done by using the Loft CLI or constructing a kube config manually. 
...
If you need to construct a kube config manually, you'll need to generate an [Access Key](../../auth/access-keys.mdx) first. Then create a `kubeconfig.yaml` in the following format:
