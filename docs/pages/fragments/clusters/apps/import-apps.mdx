...
Sometimes, it can be useful to "import" an already existing Helm chart by adding the missing labels and annotations. This has the advantage that you can view and manage the Helm release using the loft UI. Using the helper script [`import-helm-release`](https://github.com/loft-sh/loft/blob/master/hack/import-helm-release.sh) you can import Helm releases into loft.
