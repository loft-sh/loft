...
- **nginx-ingress:** allows cluster users to make their applications available on (sub-) [domains via ingresses](../../../spaces/networking.mdx)
- **cert-manager:** allows engineers to create certificates, e.g. to [enable SSL for their ingresses](../../../spaces/networking.mdx)
- **cert-issuer:** tells cert-manager to use [Let's Encrypt](https://letsencrypt.org/) for generating valid certificates
- **isolation-templates:** for [namespace isolation templates](../../../spaces/isolation.mdx)
