...
accomplished by providing any valid Agent values in the `agentHelmvalues` section of the *Loft*
chart values. These values are received by the Loft deployment and used when deploying the Agent
pods! Below is an example of the Loft values `agentHelmvalues` section.
