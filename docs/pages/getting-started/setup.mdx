...
3. If the `EXTERNAL-UP` is pending, wait a bit and go back to 2., otherwise:
...
6. If the `EXTERNAL-IP` is
   - **an IP address**, set a **A-record** for your Loft subdomain pointing to your `EXTERNAL-IP`  
