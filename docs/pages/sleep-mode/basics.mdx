...
and then you access resources within the space that will count as activity space. Some examples:
...
As you can see in the examples above, all kubernetes requests that are routed through the loft API server are counted as activity within the space and will reset the period until it sleeps.
...
There are multiple annotations you can set on a namespace to change what counts as activity within a space:
