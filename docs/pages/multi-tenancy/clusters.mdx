...
There might be cases where you don't want Loft to automatically handle kiosk updates for you and you want to manually install or upgrade kiosk. This can be achieved either by setting the environment variable `DISABLE_KIOSK` to `true` in the Loft container or by setting the annotation `loft.sh/cluster-ignore-kiosk: true` on a single or multiple connected clusters.

:::warning
If you do not install kiosk in a connected cluster, certain functionality, such as Spaces, Accounts, Account Quotas & Templates will not be available in the cluster
:::
