...
* Read/Write sensitive data in pipelines or outside of Kubernetes via Loft CLI `loft get secret my-secret.my-key my-value`
...
metadata:
  ...
  labels:
    ...
    # Optional, if the shared secret does not lie in the loft namespace
    # loft.sh/sharedsecret-namespace: my-namespace
type: Opaque
```
...
:::warning 
If you define additional keys or values in a secret that has a label `loft.sh/sharedsecret-name`, these will be automatically overridden by Loft.
:::
