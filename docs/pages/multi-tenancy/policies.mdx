...
Policies allow you to customize and extend Loft. As Loft internally uses Kubernetes resources for anything (including UI requests), you can customize almost everything with policies. 
...
spec:
  ...
  javascript: |
    ...
    if (userAccount?.metadata?.labels?.["loft.sh/user"] === "test") {
